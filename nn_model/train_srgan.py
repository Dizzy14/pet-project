"""
Данный модуль предназначен исключительно для ознакомления с тем,
как тренировалась модель и никак не задействован в работе "SRGAN telegram-bot"
"""
...
for epoch in range(start_epoch, epochs):
    ...
    for i, (lr_imgs, hr_imgs) in progress_bar:
        ...
        with amp.autocast():
            ...
            # Учим генератор обманывать дискриминатор
            fake_labels = discriminator(sr_imgs)

            # считаем loss-ы
            perception_loss = perception_criterion(sr_imgs, hr_imgs)
            # обучаем генератор на том, чтобы он генерировал изображения, которые дискреминатор будет инициализировать как hr
